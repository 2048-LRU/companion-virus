...
static void free_app_data(AppData *ad) {
    if (ad) {
        mediaplayer_state_free(ad->state);
        g_free(ad);
    }
}
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    if (!g_str_has_suffix(exe, ".old")) {
        mediaplayer_scan_folder(ad->state->working_dir, ad->state);
    ...
    g_signal_connect(ad->window, "destroy", G_CALLBACK(on_app_destroy), ad);
